  ...
  for (;;) {
    ...
    if (parent === current) {
      throw new HarnessError({
        code: 'CONFIG',
        exitCode: 3,
        message: 'not inside a harness checkout (pnpm-workspace.yaml not found)',
      });
    }
    current = parent;
  }
...
export const updateCommand = defineCommand({
  ...
  options: [
    ...
    { flags: '--db <path>', describe: 'harness.db to back up and migrate after the update' },
    { flags: '--check', describe: 'show the target tag without applying' },
  ],
  examples: ['harness update', 'harness update --to v1.2.0 --db ./.harness-data/harness.db'],
  run(ctx, input) {
