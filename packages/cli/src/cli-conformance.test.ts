...
describe('cli-conformance: every command meets the bar', () => {
  test.each(ALL_COMMANDS.map((c) => [c.name, c] as const))('%s', (_name, command) => {
    ...
    for (const ex of command.examples ?? []) {
      expect(ex.startsWith(`loom ${command.name.split(' ')[0]}`)).toBe(true);
    }
  });
  ...
  test('run exposes --reflect to enable the self-improvement loop', () => {
    const run = ALL_COMMANDS.find((c) => c.name === 'run')!;
    const flags = (run.options ?? []).map((o) => o.flags);
    expect(flags.some((f) => f.includes('--reflect'))).toBe(true);
  });
