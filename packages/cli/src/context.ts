import { loadProfile, type Profile } from '@harness/core';
...
export function createContext(options: CreateContextOptions): CliContext {
  ...
  return {
    ...
    requireProfile() {
      ...
      const dir = options.flags.profile ?? env.HARNESS_PROFILE ?? cwd;
      try {
        cachedProfile = loadProfile(dir, {
          env,
          dataDir: options.flags.dataDir ?? env.HARNESS_DATA_DIR,
        });
      } catch (error) {
        throw configError(
          error instanceof Error ? error.message : String(error),
          'run `harness init` to create a profile, or pass --profile <dir>',
        );
      }
