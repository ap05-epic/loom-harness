...
- **Mission Control is a real React SPA (`@loom/mission-control-web`) with a pod-safe vanilla fallback.** The React app (Vite + TanStack Query + Tailwind + Recharts) renders the kanban board, live worker fleet, the gate/question inbox, cost/eval charts, screen drill-down, and a **Live Crawl** view of `loom explore` — all via ~2s polling (no SSE). It's built into `dist/` by `pnpm build` (pure JS — pod-safe) and served at `/`; when the bundle is absent the server falls back to the original framework-free vanilla HTML dashboard, so `loom ui` always works. See [the cockpit](cockpit.md).
