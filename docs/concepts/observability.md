...
- **Events** (`EventLog`, append-only) — the human-readable timeline: `map.completed`, `wp.passed`, `eval.scored`, `shift.stopped`, `skill.promoted`, `memory.consolidated`, `heartbeat`, and the rest. Each event is correlated to its `run → work package → attempt`.
