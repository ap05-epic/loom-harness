...
| `loom skills list\|show\|new\|export\|import` | The project's `SKILL.md` library: list it, `show <name>` one in full, `new --name …` to author one, `export --target digit --out <dir>` for DIGIT/Copilot interop, or `import --from <dir>` external skills (see [Skills & memory](../concepts/skills-and-memory.md))                            |
